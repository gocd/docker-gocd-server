...
ARG TARGETARCH
ARG UID=1000
RUN curl --fail --location --silent --show-error "https://download.gocd.org/binaries/24.3.0-19261/generic/go-server-24.3.0-19261.zip" > /tmp/go-server-24.3.0-19261.zip && \
    unzip -q /tmp/go-server-24.3.0-19261.zip -d / && \
    mkdir -p /go-server/wrapper /go-server/bin && \
    mv -v /go-server-24.3.0/LICENSE /go-server/LICENSE && \
    mv -v /go-server-24.3.0/bin/go-server /go-server/bin/go-server && \
    mv -v /go-server-24.3.0/lib /go-server/lib && \
    mv -v /go-server-24.3.0/logs /go-server/logs && \
    mv -v /go-server-24.3.0/run /go-server/run && \
    mv -v /go-server-24.3.0/wrapper-config /go-server/wrapper-config && \
    ...
    mv -v /go-server-24.3.0/wrapper/wrapper-linux-$WRAPPERARCH* /go-server/wrapper/ && \
    mv -v /go-server-24.3.0/wrapper/libwrapper-linux-$WRAPPERARCH* /go-server/wrapper/ && \
    mv -v /go-server-24.3.0/wrapper/wrapper.jar /go-server/wrapper/ && \
