...
# download the zip file
  curl --fail --location --silent --show-error "https://download.gocd.org/experimental/binaries/18.12.0-8222/generic/go-server-18.12.0-8222.zip" > /tmp/go-server.zip && \
# unzip the zip file into /go-server, after stripping the first path prefix
  unzip /tmp/go-server.zip -d / && \
  rm /tmp/go-server.zip && \
  mv go-server-18.12.0 /go-server && \
