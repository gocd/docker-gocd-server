...
# download the zip file
  curl --fail --location --silent --show-error "https://download.gocd.org/experimental/binaries/18.11.0-8024/generic/go-server-18.11.0-8024.zip" > /tmp/go-server.zip && \
# unzip the zip file into /go-server, after stripping the first path prefix
  unzip /tmp/go-server.zip -d / && \
  rm /tmp/go-server.zip && \
  mv go-server-18.11.0 /go-server && \
