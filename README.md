...
```shell
docker pull gocd/gocd-server:v18.3.0
```

## 1.2 Build One Locally

Build one with version `18.3.0`; zip file is from GoCD [`download`][0] webpage.

```shell
GOCD_VERSION=18.3.0 \
GOCD_FULL_VERSION=18.3.0-6540
GOCD_SERVER_DOWNLOAD_URL=https://download.gocd.org/binaries/18.3.0-6540/generic/go-server-18.3.0-6540.zip \
rake build_image
```
...
```shell
docker run -d -p8153:8153 -p8154:8154 gocd/gocd-server:v18.3.0
```
...
```shell
docker run -v /path/to/godata:/godata -v /path/to/home-dir:/home/go gocd/gocd-server:v18.3.0
```
...
```shell
docker run -v /path/to/your/script.sh:/docker-entrypoint.d/your-script.sh ... gocd/gocd-server:v18.3.0
```

If you have several scripts in a directory that you'd like to execute:

```shell
docker run -v /path/to/script-dir:/docker-entrypoint.d ... gocd/gocd-server:v18.3.0
```
...
```shell
docker run -e GO_SERVER_SYSTEM_PROPERTIES="-Xmx4096mb -Dfoo=bar" gocd/gocd-server:v18.3.0
```
...
```shell
docker run --name server -it -p8153:8153 -p8154:8154 gocd/gocd-server:v18.3.0
```
