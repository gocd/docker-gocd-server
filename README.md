...
Build one with version `17.8.0`; zip file is from GoCD [`download`][0] webpage.
```bash
GOCD_VERSION=17.8.0 \
GOCD_SERVER_DOWNLOAD_URL=https://download.gocd.org/binaries/17.8.0-5277/generic/go-server-17.8.0-5277.zip \
rake build_image
```
...
```bash
docker run -d -p8153:8153 -p8154:8154 gocd/gocd-server:v17.8.0
```
...
```bash
docker run -v /path/to/godata:/godata -v /path/to/home-dir:/home/go gocd/gocd-server:v17.8.0
```
...
```bash
docker run -e GO_SERVER_SYSTEM_PROPERTIES="-Xmx4096mb -Dfoo=bar" gocd/gocd-server:v17.8.0
```
...
```bash
$ docker run --name server -it -p8153:8153 -p8154:8154 gocd/gocd-server:v17.8.0
```
