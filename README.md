...
```bash
docker pull gocd/gocd-server:v17.9.0
```

## 1.2 Build One Locally
Build one with version `17.9.0`; zip file is from GoCD [`download`][0] webpage.
```bash
GOCD_VERSION=17.9.0 \
GOCD_SERVER_DOWNLOAD_URL=https://download.gocd.org/binaries/17.9.0-5368/generic/go-server-17.9.0-5368.zip \
rake build_image
```
...
```bash
docker run -d -p8153:8153 -p8154:8154 gocd/gocd-server:v17.9.0
```
...
```bash
docker run -v /path/to/godata:/godata -v /path/to/home-dir:/home/go gocd/gocd-server:v17.9.0
```
...
```bash
docker run -e GO_SERVER_SYSTEM_PROPERTIES="-Xmx4096mb -Dfoo=bar" gocd/gocd-server:v17.9.0
```
...
```bash
$ docker run --name server -it -p8153:8153 -p8154:8154 gocd/gocd-server:v17.9.0
```
